...
import {
  ICiroHedefleriGuncellemeVerisi,
  IProfilBilgileriGuncellemeVerisi,
  ISifreDegistirmeVerisi,
  ciroHedefleriGuncellemeSemasi,
  profilBilgileriGuncellemeSemasi,
...
const ProfileField = ({
  ...
  <label className="block space-y-2.5">
    ...
    <input
      {...props}
      disabled={disabled}
      className={`h-12 w-full rounded-2xl border border-border/80 bg-background/80 px-4 text-sm text-foreground outline-none focus:border-primary/60 focus:ring-2 focus:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-70 ${interactiveClass}`}
      style={{
        boxShadow:
...
type ICiroHedefleriFormInput = z.input<typeof ciroHedefleriGuncellemeSemasi>;

const formatTargetAmount = (value: number) =>
  value > 0 ? formatMoney(value) : "Hedef yok";

export const ProfilePageClient = () => {
  const router = useRouter();
  const { user, setUser, refreshUser, logout, isLoggingOut } = useAuth();
  const [notice, setNotice] = useState<Notice>(null);
  const [isTargetEditing, setIsTargetEditing] = useState(false);

  const form = useForm<IProfilBilgileriGuncellemeVerisi>({
    resolver: zodResolver(profilBilgileriGuncellemeSemasi),
    mode: "onBlur",
  ...
  const targetForm = useForm<
    ICiroHedefleriFormInput,
    undefined,
    ICiroHedefleriGuncellemeVerisi
  >({
    resolver: zodResolver(ciroHedefleriGuncellemeSemasi),
    mode: "onBlur",
    reValidateMode: "onChange",
    shouldFocusError: true,
    defaultValues: {
      dailyCiroTarget: user?.dailyCiroTarget ?? 0,
      weeklyCiroTarget: user?.weeklyCiroTarget ?? 0,
      monthlyCiroTarget: user?.monthlyCiroTarget ?? 0,
    },
  });
  ...
  const handleTargetSubmit = targetForm.handleSubmit(async (values) => {
    ...
    try {
      const response = await fetch("/api/auth/profile/targets", {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok) {
        setNotice({ tone: "error", message: data.mesaj ?? "Ciro hedefleri güncellenemedi." });
        return;
      }
      ...
      setNotice({ tone: "success", message: "Ciro hedefleri güncellendi." });
    } catch {
      setNotice({ tone: "error", message: "Sunucu ile iletişim kurulamadı." });
    }
  });
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 p-2 lg:p-4">
        ...
        <div className="relative z-10 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-5">
            ...
            <p className="max-w-2xl text-sm leading-7 text-foreground/68 lg:text-base">
              Hesap bilgilerini burada güncelleyebilir, rolünü ve üyelik detaylarını görebilir,
              şifreni değiştirebilir ve tema ayarlarına hızlıca geçebilirsin.
            </p>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-1">
            {[
              ...
              >
                ...
                <p className="text-xs font-black uppercase tracking-[0.24em] text-foreground/60">
                  {item.label}
                </p>
      ...
      <div className="grid items-start gap-6 xl:grid-cols-2">
        <section
          className={`rounded-[2rem] border border-border/60 bg-card/82 p-6 backdrop-blur-2xl ${interactiveClass}`}
          style={{ boxShadow: cardShadow }}
        >
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-foreground/62">
                Temel kullanıcı bilgilerini buradan güncelle.
              </p>
            </div>
          </div>
        ...
        >
          <div className="mb-5 flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-black text-foreground">Ciro Hedefleri</h2>
                <p className="mt-1 text-sm text-foreground/62">
                  Dashboard hedef progress kartını bu alanlar besler.
                </p>
              </div>
            </div>
            {!isTargetEditing ? (
              <button
                ...
                className={`inline-flex h-11 items-center justify-center rounded-2xl border border-border/70 bg-background/75 px-4 text-sm font-semibold text-foreground ${interactiveClass}`}
              >
                Düzenle
              </button>
            ) : null}
          ...
          <form className="space-y-5" onSubmit={handleTargetSubmit}>
            {isTargetEditing ? (
              <>
                <div className="grid gap-4 md:grid-cols-3">
                  <ProfileField
                    ...
                    min={0}
                    step="1"
                    hint="Son 24 saat hesaplaması için"
                    error={targetForm.formState.errors.dailyCiroTarget?.message}
                    placeholder="0"
                    {...targetForm.register("dailyCiroTarget", { valueAsNumber: true })}
                  />
                  <ProfileField
                    ...
                    min={0}
                    step="1"
                    hint="Son 7 gün hesaplaması için"
                    error={targetForm.formState.errors.weeklyCiroTarget?.message}
                    placeholder="0"
                    {...targetForm.register("weeklyCiroTarget", { valueAsNumber: true })}
                  />
                  <ProfileField
                    label="Aylık Hedef"
                    type="number"
                    min={0}
                    step="1"
                    hint="Yıllık hedef dashboard'da x12 hesaplanır"
                    error={targetForm.formState.errors.monthlyCiroTarget?.message}
                    placeholder="0"
                    {...targetForm.register("monthlyCiroTarget", { valueAsNumber: true })}
                  />
                </div>
                ...
                <div className="flex flex-wrap gap-3 pt-1">
                  <button
                    ...
                    style={{ boxShadow: "0 18px 32px -20px color-mix(in oklab, var(--primary) 70%, transparent)" }}
                  >
                    {targetForm.formState.isSubmitting ? "Kaydediliyor..." : "Hedefleri Kaydet"}
                  </button>
                  <button
                    type="button"
                    onClick={() => {
              ...
              <div className="space-y-4">
                <div className="grid gap-3 md:grid-cols-3">
                  {[
                    {
                      label: "Günlük",
                      value: formatTargetAmount(user.dailyCiroTarget ?? 0),
                      hint: "Son 24 saat",
                    },
                    {
                      label: "Haftalık",
                      value: formatTargetAmount(user.weeklyCiroTarget ?? 0),
                      hint: "Son 7 gun",
                    },
                    {
                      label: "Aylık",
                      value: formatTargetAmount(user.monthlyCiroTarget ?? 0),
                      hint: "Yillik x12",
                    },
                  ].map((item) => (
                    <div
                      key={item.label}
                      className="rounded-[1.4rem] border border-border/60 bg-background/75 px-4 py-4"
                    >
                      <p className="text-xs font-black uppercase tracking-[0.18em] text-foreground/55">
                        {item.label}
                      </p>
                ...
                <p className="text-xs leading-6 text-foreground/58">
                  Dashboard hedef progress kartı seçili gün, hafta, ay ve yıl görünümlerinde bu
                  hedefleri kullanır.
                </p>
              </div>
            )}
        ...
        >
          <div className="mb-7 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-foreground/62">
                Mevcut şifreni doğrula ve yeni şifre belirle.
              </p>
            </div>
          </div>
        ...
        >
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex items-center justify-between rounded-2xl bg-background/75 px-4 py-3">
              <span className="text-foreground/62">Guncellendi</span>
        ...
        >
          <div className="mb-7 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-foreground/62">
                Profil dışındaki temel kullanıcı aksiyonları.
              </p>
            </div>
          </div>
