...
export async function PUT(istek: NextRequest, { params }: IRouteParams) {
  try {
    ...
    const guncellenenKisi = await UserModel.findByIdAndUpdate(params.id, dogrulamaSonucu.data, { new: true });
...
export async function DELETE(istek: NextRequest, { params }: IRouteParams) {
  try {
    await veritabaninaBaglan();
    const silinenKisi = await UserModel.findByIdAndDelete(params.id);
